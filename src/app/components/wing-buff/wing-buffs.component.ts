...
@Component({
  selector: 'app-ag-wing-buff-origin-renderer',
  template: `@if (origin) { <a class="atmos-text-link" [routerLink]="origin.route">{{ origin.name }}</a> }`,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [RouterLink]
})
class AgWingBuffOriginRendererComponent implements ICellRendererAngularComp {
  origin?: IOriginLink;
  ...
  readonly colDefs: ColDef[] = [
    {
      ...
      cellRenderer: AgSpiritLinkRendererComponent
    },
    {
      headerName: 'Type',
      width: 100,
    ...
    },
    {
      ...
      cellRenderer: AgWingBuffOriginRendererComponent
    },
    {
      headerName: 'Unlocked',
      width: 140,
