...
export interface SpiritTreeNodeClickEvent {
  node: INode;
  event: MouseEvent;
}
 ...
 */
export type TreeEditSlotKind = 'add' | 'link' | 'unlink';

export interface TreeEditSlot {
  position: NodePosition;
  /** Level within the column, root = 0. -1 is the slot below the root. */
  level: number;
  kind: TreeEditSlotKind;
  /** Direction relative to the node being edited, echoed back on click. */
  direction: 'nw' | 'n' | 'ne' | 'below';
}

interface TreeRenderSlot {
  node?: INode;
  edit?: TreeEditSlot;
}

interface TreeRenderColumn {
  position: NodePosition;
  slots: ReadonlyArray<TreeRenderSlot>;
  /**
 ...
 */
const sharedNodeAction = signal<NodeAction>('unlock');

/**
 * Spirit-tree renderer. Lays the tree out as a vertical stack of 3-column
 * rows, delegating each node to `NodeComponent`.
 *
...
@Component({
  ...
  styleUrl: './spirit-tree.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [NodeComponent, CostComponent, DateComponent, MatIcon, RouterLink, DraftWarningComponent, NgTemplateOutlet, TooltipDirective]
})
export class SpiritTreeComponent {
  ...
  readonly nodeClicked = output<SpiritTreeNodeClickEvent>();
  readonly editSlotClicked = output<TreeEditSlot>();
  ...
  readonly rows = computed<ReadonlyArray<TreeRow>>(() => {
    const t = this.tree();
    const rows: TreeRow[] = [];
   ...
   */
  readonly columns = computed<TreeColumns>(() => {
    const cols: TreeColumns = { left: [], center: [], right: [] };
    const t = this.tree();
   ...
   * arrangement.
   */
  readonly orderedColumns = computed<ReadonlyArray<{ position: NodePosition; nodes: ReadonlyArray<INode | undefined> }>>(() => {
   ...
   */
  readonly renderColumns = computed<ReadonlyArray<TreeRenderColumn>>(() => {
    const slots = this.editSlots();
    const byPosition = new Map<string, TreeEditSlot>();
    ...
    return this.orderedColumns().map(col => {
      ...
      const rendered: Array<TreeRenderSlot> = [];
  ...
  onEditSlotClicked(slot: TreeEditSlot, event: MouseEvent): void {
    event.preventDefault();
    event.stopPropagation();
    this.editSlotClicked.emit(slot);
  }

  editSlotTitle(slot: TreeEditSlot): string {
    switch (slot.kind) {
  ...
  readonly action = computed<NodeAction>(() => this.forceNodeAction() ?? sharedNodeAction());
  ...
  setAction(action: NodeAction): void {
    sharedNodeAction.set(action);
  }
