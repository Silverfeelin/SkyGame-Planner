...
import {
  IEvent, IEventInstance, IEventInstanceSpirit, ISpiritTree, INode, IShop,
  IItemListNode, ICalculatorDataTimedCurrency, ICalculatorData
} from 'skygame-data';
...
import { AtmosCheckboxComponent, AtmosDraftWarningComponent, AtmosItemListComponent, AtmosSpiritTreeComponent } from '@app/redesign/shared/atmos-shared-widgets';
...
@Component({
  selector: 'app-atmos-event-calculator',
  templateUrl: './atmos-event-calculator.component.html',
  styleUrl: './atmos-event-calculator.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [RouterLink, MatIcon, DateTimePipe, AtmosCheckboxComponent, AtmosSpiritTreeComponent, AtmosItemListComponent, AtmosEventQuickActionsComponent, AtmosDraftWarningComponent, AtmosQuickActionsComponent]
})
export class AtmosEventCalculatorComponent implements OnInit {
  ...
  listNodeShowButtons?: IItemListNode;

  constructor() {
    this.guid = this._route.snapshot.queryParamMap.get('guid') || '';
    let instance: IEventInstance | undefined;
    if (this.guid) {
      instance = this._dataService.guidMap.get(this.guid) as IEventInstance;
    } else {
      const eventInstances = this._dataService.eventConfig.items
        .map(e => DateHelper.getActive(e.instances))
        .filter(e => e) as Array<IEventInstance>;
  ...
  wantNode(node: INode, event: MouseEvent): void {
    ...
    if (this.wantNodes[node.guid]) { delete this.wantNodes[node.guid]; }
    else { this.wantNodes[node.guid] = node; }
    this.calculate();
    this.saveSettings();
  }
  ...
  wantListNode(node: IItemListNode, event: MouseEvent): void {
    ...
    if (this.wantListNodes[node.guid]) { delete this.wantListNodes[node.guid]; }
    else { this.wantListNodes[node.guid] = node; }
    this.calculate();
    this.saveSettings();
  }
  ...
  currencyInputChanged(evt: Event): void {
    const target = evt.target as HTMLInputElement;
    if (target === this.inpEc.nativeElement) { this.currencyCount = this._currencyService.clamp(parseInt(target.value, 10) || 0); }
    if (target === this.inpC.nativeElement) { this.candleCount = this._currencyService.clamp(parseInt(target.value, 10) || 0); }
    if (target === this.inpH.nativeElement) { this.heartCount = this._currencyService.clamp(parseInt(target.value, 10) || 0); }
  ...
  selectEverything(): void {
    ...
    this.wantListNodes = {};
    this.allListNodes.forEach(n => { this.wantListNodes[n.guid] = n; });
    this.calculate();
    this.saveSettings();
  }
  ...
  private loadSettings(): void {
    ...
    const parsed = JSON.parse(data) || { tc: {}, wn: [], ln: [] };
    ...
    this.timedCurrencies.forEach(tc => { this.timedCurrencyCount[tc.guid] ||= 0; });
  ...
  private calculate(): void {
    ...
    for (const timedCurrency of this.timedCurrencies) {
      ...
      if (timedCurrency.endDate >= date) { this.currencyAvailable += available; }
    }
