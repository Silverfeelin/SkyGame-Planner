...
@Component({
  selector: 'app-atmos-children-of-light',
  templateUrl: './atmos-children-of-light.component.html',
  styleUrl: './atmos-children-of-light.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [MapInstanceService],
  imports: [MatIcon, AgGridAngular, AtmosWingedLightQuickActionsComponent, AtmosQuickActionsComponent]
})
export class AtmosChildrenOfLightComponent implements AfterViewInit, OnDestroy {
  ...
  readonly colDefs: ColDef[] = [
    {
      ...
      cellRenderer: (p: any) => {
        ...
        return `<span class="atmos-col__map-icon" onclick="atmosColShowArea('${row.area.guid}')"><span class="maticon">location_on</span></span>`;
      }
    },
    {
      headerName: 'Found',
      ...
      cellRenderer: (p: any) => {
        ...
        const cls = row.unlocked === row.wl.length
          ? 'atmos-col__row-complete' : row.unlocked ? 'atmos-col__row-partial' : '';
        return `<span class="atmos-col__row-unlocked ${cls}">${row.unlocked} / ${row.wl.length}</span><span class="maticon atmos-col__row-toggle" onclick="atmosColToggleRow('${row.area.guid}')">lock_open</span>`;
      }
    },
    {
      field: 'area',
  ...
  constructor(
    ...
    this.wingedLights.forEach(wl => {
      ...
      const row: IRow = {
        ...
        unlocked: (area.wingedLights || []).filter(w => w.unlocked).length
      };
      row.wl.forEach(w => { this.rowMap[w.guid] = row; });
      this.rows.push(row);
    });
  ...
  ngAfterViewInit(): void {
    ...
    this.map = this._mapInstanceService.initialize(this.mapContainer.nativeElement.querySelector('.atmos-col__map'), mapInit);
    ...
    this.wingedLights.filter(w => w.mapData?.position).forEach(w => {
      if (!w.mapData) { return; }
      const obj: IMapWingedLight = { wl: w };

      obj.marker = L.marker(w.mapData.position!, {
        icon: wlIcon,
        opacity: w.unlocked ? 0.4 : 1,
      });

      const popup = new L.Popup({
        content: () => this.createPopup(w),
    ...
    (window as any).atmosColShowArea = (g: string) => {
      this._zone.run(() => {
        const row = this.rows.find(r => r.area.guid === g);
        if (row) { this.showArea(row); }
      });
    };
    (window as any).atmosColToggleRow = (g: string) => {
      this._zone.run(() => {
        const row = this.rows.find(r => r.area.guid === g);
        if (row) {
          this.toggleRow(row);
  ...
  ngOnDestroy(): void {
    ...
    delete (window as any).atmosColShowArea;
    delete (window as any).atmosColToggleRow;
...
<button class="s-leaflet-hasicon" data-direction="right" type="button" onclick="nextCol('${wl.guid}', 1)" title="Go to next light.">
  ...
  private updateTable(wl: IWingedLight): void {
    ...
    if (!row) { return; }
    row.unlocked = (row.wl || []).filter(w => w.unlocked).length;
  ...
  private markCol(div: HTMLElement, guid: string, next: boolean): void {
    ...
    this.updateMarker(wl);
    this.updateTable(wl);
    if (next) { this.nextCol(guid, true); }
  }
